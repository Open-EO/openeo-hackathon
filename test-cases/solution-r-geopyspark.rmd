...
```{r}
# NDVI currently not supported by the back-end, try it for filter_bbox instead
con %>% describeProcess("filter_bbox")
```
...
```{r}
sentinel2 = "CGS_SENTINEL2_RADIOMETRY_V101"
# Currently not supported by the back-end
...
# con %>% executeTask(task=task, format = "GTiff", output_file = "task3.tiff")
```
...
Construct and execute the process graph. Downloads the file in JSON format with the file name `task4.json`:
```{r}
# Currently not supported by the back-end
...
# con %>% executeTask(task=task5, format="JSON", output_file = "task4.json")
```
